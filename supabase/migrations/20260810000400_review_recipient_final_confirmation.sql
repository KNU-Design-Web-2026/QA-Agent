-- A designer explicitly selected for review can make the final visual decision.
...
begin
  ...
  select exists(
    select 1 from public.qa_review_recipients recipient
    where recipient.qa_comment_id = comment_id and recipient.reviewer_id = actor_id
  ) into is_review_recipient;
  ...
  if not (
    ...
    or (current_comment.status = 'review_requested' and next_status = 'done' and (is_author or is_review_recipient or actor_role = 'admin'))
    or (current_comment.status = 'review_requested' and next_status = 'in_progress' and (is_author or is_review_recipient or actor_role = 'admin'))
    or (current_comment.status = 'done' and next_status = 'open' and (is_author or is_review_recipient or actor_role = 'admin'))
  ) then
