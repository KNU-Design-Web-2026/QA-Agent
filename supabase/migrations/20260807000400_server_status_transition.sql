-- The QA Hub authenticates with its own signed cookie. This RPC preserves the
-- immutable event history while authorizing that cookie's user explicitly.
...
begin
  ...
  if not exists (
    ...
    where m.organization_id = org_id and m.user_id = actor_id
      and m.role = any(array['admin','designer','developer']::public.member_role[])
  ) then raise exception 'Not allowed to transition this QA comment'; end if;
...
revoke execute on function public.transition_qa_comment_as_actor(uuid, public.qa_status, uuid, text) from public, anon, authenticated;
grant execute on function public.transition_qa_comment_as_actor(uuid, public.qa_status, uuid, text) to service_role;
